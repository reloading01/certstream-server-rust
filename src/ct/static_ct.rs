...
pub async fn run_static_ct_watcher(log: CtLog, ctx: WatcherContext) {
    ...
    let counter_tiles = metrics::counter!("certstream_static_ct_tiles_fetched", "log_id" => log_id.clone());
    let counter_parse_failures = metrics::counter!("certstream_static_ct_parse_failures", "log_id" => log_id.clone());
    let counter_entries_parsed = metrics::counter!("certstream_static_ct_entries_parsed", "log_id" => log_id.clone());
    ...
    loop {
        ...
        if raw_tree_size < high_water_tree_size {
            ...
            metrics::counter!("certstream_static_ct_tree_size_rollbacks", "log_id" => log_id.clone()).increment(1);
            health.record_failure(config.unhealthy_threshold);
            sleep(health.get_backoff()).await;
            continue;
        }
